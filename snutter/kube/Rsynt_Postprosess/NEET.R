...
# deletestrata <- data.table::dcast(deletestrata, KOMMUNE + AAR + KJONN + ALDER + UTDANN + INNVKAT ~ MALTALL, value.var = "UKJENT")
...
# deletestrata <- deletestrata[sumTELLER > 0.08 | abs(DIFF) > 0.05, .(KOMMUNE, AAR, KJONN, ALDER, UTDANN, INNVKAT)]
...
# KUBE <- collapse::join(KUBE, deletestrata, multiple = "T", on = c("KOMMUNE", "AAR", "KJONN", "ALDER", "UTDANN", "INNVKAT"), overid = 2, verbose = F)
...
# KUBE[spv_tmp %in% c(0,4) & SLETT == 1, (flags) := 1L]
# KUBE[, SLETT := NULL]
