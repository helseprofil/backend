#' @title update_khfunctions_dimlist
#' @description 
#' Leser TAB1-3-kolonnene i ACCESS og lager en liste over alle potensielle dimensjonskolonner
#' Brukes for å identifisere dimensjoner og verdier. Returnerer en liste som kan limes rett inn i `config-khfunctions.yml`
#' @noRd
update_khfunctions_dimlist <- function(){
  con <- qualcontrol:::ConnectKHelsa()
  on.exit(RODBC::odbcClose(con), add = TRUE)
  date <- qualcontrol:::SQLdate(Sys.time())
  standarddimensions <- c("GEOniv", "GEO", "FYLKE", "AAR", "AARl", "AARh", "ALDER", "ALDERl", "ALDERh", "KJONN", "UTDANN", "INNVKAT", "LANDBAK")
