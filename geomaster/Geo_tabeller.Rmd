...
KnrHarmOld <- data.table::setDT(RODBC::sqlQuery(con, "SELECT * FROM KnrHarm", as.is = TRUE))
GeoKoderOld <- data.table::setDT(RODBC::sqlQuery(con, "SELECT * FROM GeoKoder", as.is = TRUE))
```
...
- Kan skrives til ACCESS og brukes derfra
- Må tilpasse funksjonen til å skrive til ACCESS
```{r}
source("https://raw.githubusercontent.com/helseprofil/backend/main/geomaster/R/lks_startaar.R")
d <- get_lks_startaar(bef_datotag = "2026-05-27-12-21", max_endring = 0.15)
